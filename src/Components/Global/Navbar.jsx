...
import { Star, MessageCircle, LayoutDashboard, User, Home, X, ChevronDown, Menu } from "lucide-react"
            ...
            <div className="flex flex-col items-center gap-4 mt-8">
                    ...
                    <button
                      className={`${
                        isActive
                          ? "bg-white text-blue-600 shadow-lg scale-105"
                          : "text-white hover:bg-white hover:bg-opacity-20 hover:scale-105"
                      } flex items-center space-x-2 px-8 py-3 rounded-full text-lg w-full transition-all duration-300 ease-in-out transform`}
                    >
              ...
              {user && (
                ...
                >
                  ...
                  >
                    <dashboardTab.icon className="h-5 w-5" />
                    <span className="font-medium">{dashboardTab.label}</span>
                  </button>
                </Link>
              )}
              <div className="w-full flex justify-center mt-4">
                {user ? (
                  <div className="relative w-full flex flex-col items-center">
                    <button
                      onClick={handleProfileClick}
                      className="flex items-center space-x-2 bg-white bg-opacity-20 rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 w-full justify-center"
                    >
                      <User className="h-6 w-6 text-white" />
                      <span className="text-white font-semibold max-w-[100px] truncate">{user.username}</span>
                      <ChevronDown className={`h-4 w-4 text-white transition-transform duration-200 ${profileDropdownOpen ? 'rotate-180' : ''}`} />
                    ...
                    {profileDropdownOpen && (
                      <div className="absolute left-1/2 -translate-x-1/2 mt-2 w-48 bg-white rounded-xl shadow-lg py-2 z-50 animate-fade-in">
                        <div className="px-4 py-2 text-gray-700 font-semibold border-b max-w-full truncate" title={user.email}>{user.email}</div>
                        <button
                          onClick={handleLogout}
                          className="w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 font-medium rounded-b-xl transition-colors duration-200"
                  ...
                  <button
                    ...
                    className="bg-white bg-opacity-20 hover:bg-opacity-30 rounded-full p-2 transition-all duration-200 hover:scale-110 w-full flex justify-center"
                  >
                    <User className="h-6 w-6 text-white" />
                  </button>
                )}
